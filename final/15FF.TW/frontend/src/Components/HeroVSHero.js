...
const HeroVSHeroTable = ({versusData}) => {
    ...
    const columns = [
        {
            title: hero_map[versusData.name[0]],
            dataIndex: 'Hero1',
            key: 'Hero1',
            align: 'center',
            render: (text, record) => {
        ...
        {
            title: hero_map[versusData.name[1]],
            dataIndex: 'Hero2', 
            key: 'Hero2',
            align: 'center',
            render: (text, record) => {
        ...
        <div className="HeroVSHero-Table-Top">
            ...
            <div className="HeroVSHero-Top-MiddlePad">
                <div className="HeroVSHero-NameTag"> 
                    <Tag color='orange' style={{textAlign: 'center', width:'180px', fontSize:'20px', height: '25px'}}>
                        {hero_map[versusData.name[0]]}</Tag>
                    <p style={{fontSize: '20px', fontWeight: "bold"}}>VS.</p>
                    <Tag color='orange' style={{textAlign: 'center', width:'180px', fontSize:'20px', height: '25px', position:'relative',right:'-8px'}}>
                        {hero_map[versusData.name[1]]}</Tag>
                </div>
                <div className="HeroVSHero-WinRate">
                    <Tag color={(versusData.win_rate[0] > versusData.win_rate[1])?'green':'red'} style={{fontSize: '20px'}}>{versusData.win_rate[0]+'%'}</Tag>
                    <Tag color='orange' style={{textAlign: 'center', width:'130px', fontSize:'20px', height: '25px', position:'relative',right:'-5px'}}>Win Rate</Tag>
                    <Tag color={(versusData.win_rate[1] > versusData.win_rate[0])?'green':'red'} style={{fontSize: '20px', position:'relative',right:'-8px'}}>{versusData.win_rate[1]+'%'}</Tag>
                </div>
            </div>
...
export default HeroVSHeroTable