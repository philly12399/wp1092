...
const ChatRoom = ({ me, displayStatus }) => {
  ...
  // const { messages, fetchData, setMessages } = useClient();
  let messages;
  const myRef = useRef();
  const otherRef = useRef();
  const newRef = useRef();
  const [startChat] = useMutation(CREATE_CHATBOX_MUTATION);
  const [sendMessage] = useMutation(ADD_MSG_MUTATION);
  const { loading, error, data, subscribeToMore } = useQuery(MSG_QUERY, {
    variables: { me: me, friend: newRef.current },
  });
  useEffect(() => {
    console.log(data);
  }, [data]);
    ...
    <>
      ...
      <Input.Search
        ...
        onSearch={(msg) => {
          ...
          sendMessage({ variables: { me: me, friend: myRef.current } });
          setMessageInput("");
