...
class TodoMain extends Component {
    ...
    newItem = (Id,Detail,Checked)=>{
        ...
        return(
            <li class="todo-app__item">
                ...
                <img src="./img/x.png" class="todo-app__item-x" id={Id} onClick={e=>this.handleDel(e.target.id)}></img>
            </li>
        );
    }
    id2Ind = (id)=>{
    ...
    handleAdd = (e) =>{
        if(e.key === "Enter" && e.target.value !== "" ){
            ...
            var newList = this.state.allList.concat([new this.todoItem(this.state.listId,l,e.target.value)]);
    ...
    render() {
        if(this.props.clear){
            var  delList =[];
            for(let i = 0;i < this.state.allList.length;i++) {
                if(this.state.allList[i].checked == 1) delList = delList.concat(this.state.allList[i].Id);
            }
        ...
        return (
            ...
            <input class='todo-app__input' placeholder="What needs to be done?" onKeyPress={e => this.handleAdd(e)}></input>
